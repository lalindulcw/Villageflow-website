...
// Handles: Card, Bank Transfer, Mobile Payment, Offline Payment
const PaymentModal = ({ amount, certType, onSuccess, onCancel, lang }) => {
    ...
    const handlePayment = async () => {
        ...
        let success = false;
        if (paymentMethod === 'card') success = await processCardPayment();
        else if (paymentMethod === 'bank') success = await processBankTransfer();
        else if (paymentMethod === 'mobile') success = await processMobilePayment();
        else if (paymentMethod === 'offline') success = await processOfflinePayment();
        setIsProcessing(false);
        if (success) setPaymentStep('receipt');
    };
                    ...
                    <div style={styles.modalBody}>
                        <div style={styles.receiptContainer}>
                            ...
                            <div style={styles.receiptDetails}>
                                ...
                                <div style={styles.receiptRow}><span>{t.fee}:</span><strong style={{color: '#800000'}}>Rs. {amount}/=</strong></div>
                    ...
                    <div style={styles.modalBody}>
                        {paymentMethod === 'card' && (
                            <div>
                                ...
                                <div style={styles.inputGroup}>
                                    ...
                                    <span style={styles.demoHint}>Demo: Enter any 16-digit number</span>
                                </div>
                                <div style={styles.inputGroup}>
                        ...
                        {paymentMethod === 'mobile' && (
                            <div>
                                ...
                                <div style={styles.paymentInfoNote}><Smartphone size={14} /><span>Demo: Any PIN works</span></div>
                            </div>
                        )}
                        {paymentMethod === 'offline' && (
                            <div>
        ...
        <div style={styles.modalOverlay}>
            <div className="modal-content" style={styles.modalContent}>
                ...
                <div style={styles.modalBody}>
                    ...
                    <div style={styles.paymentAmount}><span>{t.fee}:</span><strong style={{color: '#800000', fontSize: '24px'}}>Rs. {amount}/=</strong></div>
                    ...
                    {paymentMethod === 'offline' && (
                        <div style={styles.offlineInfoBox}>
                            ...
                            <p>{t.offlinePaymentDesc}</p>
                            <p style={{fontSize: '12px', marginTop: '10px', color: '#800000'}}><strong>{t.referenceNumber}:</strong> VF{Date.now()}</p>
                        </div>
                    )}
                    <div style={styles.demoNote}><Info size={14} color="#f59e0b" /><span>Demo Mode: Any valid-looking input works.</span></div>
                </div>
                <div style={styles.modalFooter}>
        ...
        <div style={styles.previewContainer}>
            ...
            <div style={styles.previewContent}>
                ...
                : file.type === 'application/pdf' ? (
                    <div style={styles.pdfPreview}><FileText size={40} color="#800000" /><span>{file.name}</span></div>
                ) : (
...
// MAIN COMPONENT: Member2Certificates
function Member2Certificates() {
    ...
    const [nic, setNic] = useState('');                           // Family member NIC
    const [certType, setCertType] = useState('Residency Certificate');  // Certificate type
    const [selectedFile, setSelectedFile] = useState(null);       // Uploaded utility bill
    const [isScanning, setIsScanning] = useState(false);          // NIC scanning status
    ...
    const [ownerVerified, setOwnerVerified] = useState(false);    // Owner NIC verified?
    const [myApplications, setMyApplications] = useState([]);     // User's application history
    
    const [applyFor, setApplyFor] = useState('Self');             // Self or Family member
    const [isChild, setIsChild] = useState(false);                // Is applicant under 18?
    const [memberName, setMemberName] = useState('');              // Family member name
    const [relationship, setRelationship] = useState('');          // Relationship to applicant
    const [memberAge, setMemberAge] = useState(null);              // Age of family member
    const [memberExists, setMemberExists] = useState(false);       // Member exists in DB?
    const [nicValidFormat, setNicValidFormat] = useState(false);   // NIC format valid?
    
    const [editId, setEditId] = useState(null);                    // ID being edited
    const [scanMessage, setScanMessage] = useState({ text: '', type: '' });   // Scan result message
    const [statusMsg, setStatusMsg] = useState({ text: '', type: '' });        // Status message
    const [memberValidationMsg, setMemberValidationMsg] = useState({ text: '', type: '' }); // Member validation
    
    const [showPayment, setShowPayment] = useState(false);          // Show payment modal
    const [paymentCompleted, setPaymentCompleted] = useState(false); // Payment done?
    const [paymentData, setPaymentData] = useState(null);            // Payment details
    const [isOnline, setIsOnline] = useState(navigator.onLine);      // Online status
    const [pendingSyncCount, setPendingSyncCount] = useState(0);     // Offline apps pending
    ...
    const fetchMyApps = useCallback(async () => {
        if (!owner?._id) return;
        try {
            const res = await axios.get(`https://villageflow.onrender.com/api/certificates/user/${owner._id}`);
            if (res.data) {
    ...
    const syncOfflineApplications = useCallback(async () => {
        ...
        for (const app of offlineApps) {
            try {
                ...
                await axios.post('https://villageflow.onrender.com/api/certificates/apply', formData);
            } catch (err) {
    ...
    const validateFamilyMember = useCallback(async (nicValue) => {
        ...
        try {
            const res = await axios.get(`https://villageflow.onrender.com/api/certificates/citizens/check/${nicValue}`);
    ...
    const submitWithPaymentData = async (paymentInfo) => {
        ...
        try {
            const url = editId 
                ? `https://villageflow.onrender.com/api/certificates/update/${editId}` 
                : `https://villageflow.onrender.com/api/certificates/apply`;
    ...
    const submitApplication = async () => {
        ...
        try {
            const url = editId ? `https://villageflow.onrender.com/api/certificates/update/${editId}` : `https://villageflow.onrender.com/api/certificates/apply`;
            await axios({ method: editId ? 'put' : 'post', url: url, data: formData, headers: { 'Content-Type': 'multipart/form-data' } });
            setStatusMsg({ text: editId ? t.updateSuccess : t.submitSuccess, type: 'success' });
            setPaymentCompleted(false);
            setPaymentData(null);
    ...
    const downloadCertificate = async (app) => {
        const doc = new jsPDF();
        const verifyURL = `https://villageflow.onrender.com/verify-cert/${app._id}`;
    ...
    return (
        <div className="member2-certificates" style={styles.pageContent}>
                ...
                <div className="member2-main-card card" style={styles.card}>
                    ...
                    {!ownerVerified && !editId ? (
                        <div className="auth-zone" style={styles.authZone}>
                            <div style={styles.scanIllustration}><Camera size={40} color="#800000" /></div>
                                ...
                                <div className="family-panel" style={styles.familyPanel}>
                                    ...
                                    {(!isChild && memberExists && memberAge !== null && memberAge >= 18) && (
                                        <div style={styles.memberDetailsCard}>
                                            <div style={styles.memberDetailsHeader}><User size={16} color="#800000" /><span style={{fontWeight: '600'}}>{t.memberDetails}</span></div>
                                    ...
                                    <div className="member2-action-row" style={styles.actionRowProfessional}>
                                        {app.status === 'Pending' && (
                                            <div style={{display:'flex', gap:'10px', flexWrap:'wrap'}}>
                                                ...
                                                <button onClick={async() => {if(window.confirm(t.deleteConfirm)) {await axios.delete(`https://villageflow.onrender.com/api/certificates/delete/${app._id}`); fetchMyApps();}}} style={styles.deleteBtnProfessional}><Trash2 size={16}/></button>
                                            </div>
                                        )}
                                        {app.status === 'Approved' && (
                                            <div style={{display:'flex', gap:'10px', flexWrap:'wrap'}}>
            ...
            <style>{`
                ...
                .member2-certificates .upload-strip:hover { background-color: #f8fafc !important; border-color: #800000 !important; }
            `}</style>
        </div>
    );
}
...
const styles = {
    ...
    modalHeader: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '20px', background: '#800000', color: 'white' },
    ...
    paymentBtn: { flex: 1, background: 'linear-gradient(135deg, #800000 0%, #a00000 100%)', color: 'white', border: 'none', padding: '14px', borderRadius: '12px', fontWeight: '700', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px' },
