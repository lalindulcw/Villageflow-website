...
function WelfarePage() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (isEditing) {
                await axios.put(`https://villageflow.onrender.com/api/welfare/update/${isEditing}`, formData);
                setStatusMessage({ type: 'success', message: 'සාර්ථකව යාවත්කාලීන කළා' });
            } else {
                await axios.post('https://villageflow.onrender.com/api/welfare/add', formData);
    ...
    const handleDelete = async (id) => {
        ...
        setSubmitting(true);
        try {
            await axios.delete(`https://villageflow.onrender.com/api/welfare/delete/${id}`);
            fetchBeneficiaries();
    ...
    const toggleStatus = async (id, currentStatus) => {
        ...
        setSubmitting(true);
        try {
            await axios.put(`https://villageflow.onrender.com/api/welfare/approve/${id}`, { status: newStatus });
            fetchBeneficiaries();
    ...
    const viewPaySlip = (paySlipPath, fullName) => {
        ...
        const fileUrl = `https://villageflow.onrender.com/api/welfare/pay-slip/${filename}`;
