...
function NoticePage() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (isEditing) {
                await axios.put(`https://villageflow.onrender.com/api/notices/update/${isEditing}`, formData);
                alert("නිවේදනය යාවත්කාලීන කළා!");
            } else {
                await axios.post('https://villageflow.onrender.com/api/notices/add', formData);
    ...
    const handleDelete = async (id) => {
        if (window.confirm("මෙම නිවේදනය මකා දැමීමට අවශ්‍යද?")) {
            await axios.delete(`https://villageflow.onrender.com/api/notices/delete/${id}`);
            fetchNotices();
        }
    };
