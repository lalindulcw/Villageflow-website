...
function Profile() {
    ...
    const fetchUserData = useCallback(async (id) => {
        try {
            const res = await axios.get(`https://villageflow.onrender.com/api/auth/user/${id}`);
    ...
    const handleUpdate = async () => {
        ...
        try {
            const targetId = user._id || user.id;
            const res = await axios.put(`https://villageflow.onrender.com/api/auth/update/${targetId}`, editData);
    ...
    const handleDelete = async () => {
        if (window.confirm(t.confirmDelete)) {
            try {
                const targetId = user._id || user.id;
                await axios.delete(`https://villageflow.onrender.com/api/auth/delete/${targetId}`);
    ...
    const handleProxyRegister = async (e) => {
        ...
        try {
            ...
            const response = await axios.post('https://villageflow.onrender.com/api/auth/proxy-register', submitData, {
            ...
            if (err.response) {
                ...
                setProxyErrors({ general: lang === 'si' ? "සේවාදායකයට සම්බන්ධ විය නොහැක. Backend server එක (https://villageflow.onrender.com) ක්‍රියාත්මකද පරීක්ෂා කරන්න." : (lang === 'ta' ? "சேவையகத்துடன் இணைக்க முடியவில்லை. Backend server (https://villageflow.onrender.com) இயங்குகிறதா சரிபார்க்கவும்." : "Cannot connect to server. Check whether backend server (https://villageflow.onrender.com) is running.") });
            } else {
